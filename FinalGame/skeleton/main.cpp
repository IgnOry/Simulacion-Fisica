...
void updateConsole() //X y Z cambiados por la camara
{
	...
	std::cout << falcon->getAcc().x*100 << " " << falcon->getAcc().y*100 << " " << falcon->getAcc().z*100 << std::endl;
...
void collisionSystem()
{
	...
	if (detectCollisions(falcon->getParticle(), target->getPart()))
	{
		...
		services_.getAudios()->playChannel(Resources::Victory, 0);
...
void stepPhysics(bool interactive, double t)
{
	if (playing)
	{
		...
		{
			services_.getAudios()->playChannel(Resources::Lost, 0);
			playing = false;
			win = false;
		}
	}
	else 
	{
		...
		{
			system("CLS");
			std::cout << "Vaya, parece que esto no es lo tuyo" << std::endl;
			deleteStuff();
		}
	}
	/////////////////////////////////////////////////////////////////////////
