...
void initPhysics(bool interactive)
{
	...
	spring = new ParticleSpring(1.0, 1.0, Vector3(15, 15, 15));
	p = new Particle(1.0, Vector4(1.0,1.0,1.0,1.0), Vector3(15,15,15), 50.0);
	particlesVec.push_back(p);
	grav = new ParticleGravity(Vector3(0, -1, 0));
	pfRegistry->add(p, spring);
	pfRegistry->add(p, grav);

	spring1 = new ParticleSpring(1.0, 1.0, Vector3(0, 5, 0));
	p1 = new Particle(1.0, Vector4(0.0, 1.0, 1.0, 1.0), Vector3(0, 5, 0), 50.0);
	p2 = new Particle(1.0, Vector4(0.0, 0.0, 1.0, 1.0), Vector3(0, 0, 0), 50.0);
	particlesVec.push_back(p1);
	particlesVec.push_back(p2);
	pfRegistry->add(p1, spring1);
	pfRegistry->add(p2, spring1);
	pfRegistry->add(p1, grav);
	pfRegistry->add(p2, grav);

	WindGenerator* wind2 = new WindGenerator(Vector3(5, 5, 5), Vector3 (0, 5, 0), 30.0);
	pfRegistry->add(p2, wind2);

	flote = new ParticleBuoyancy(-5, 5, 5);
	p3 = new Particle(1.0, Vector4(0.0, 1.0, 0.0, 1.0), Vector3(20, 5, 20), 50.0);
	particlesVec.push_back(p3);
	pfRegistry->add(p3, grav);
	pfRegistry->add(p3, flote);
...
void keyPress(unsigned char key, const PxTransform& camera)
{
	...
	case ' ':
	{
		wind = new WindGenerator(Vector3(5, 5, 5), Vector3(5, 5, 5), 30);
		auto auxP = particlesVec.begin();
		while (auxP != particlesVec.end()) {
			Particle* p = (*auxP);
			pfRegistry->add(p, wind);
			auxP++;
		}
		break;
	}
